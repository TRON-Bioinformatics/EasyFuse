...
class IOmethods(object):
    """collection of i/o relevant methods"""
    @classmethod
    def create_folder(cls, folder, logger):
        """This function creates a specified folder, if not already existing and grants the respective permission."""
        if not os.path.exists(folder):
            os.makedirs(folder)
            cls.grant_permissions(folder, stat.S_IRWXU | stat.S_IRWXG | stat.S_IROTH | stat.S_IXOTH)
            logger.debug("Created folder {}".format(folder))
        else:
            logger.debug("Folder {} already exists".format(folder))

    @classmethod
    def create_folder_wo_logging(cls, folder):
        """This function creates a specified folder, if not already existing and grants the respective permission."""
        if not os.path.exists(folder):
            os.makedirs(folder)
            cls.grant_permissions(folder, stat.S_IRWXU | stat.S_IRWXG | stat.S_IROTH | stat.S_IXOTH)

    # note urla: this has obviously no effect if you are on a windows file system, but should not raise errors there (not sure for OSX)
    @staticmethod
    ...
    @classmethod
    def get_fastq_files(cls, input_path, logger):
        ...
        for i, fq_file in enumerate(sorted(fastqs)):
            ...
            try:
                ...
                #forrev = re.search('(\_|\_R)([1-2])(\_|\.f)', cls.path_leaf(fq_file)).group(2)
                # urla: replaced original (up) with the following <- worked in minimal test set, should be fine for everything
                forrev = re.search('_R?([1-2])(_|[.]f)', cls.path_leaf(fq_file)).group(1)
                print(forrev)
        ...
        if right:
            for _, i in enumerate(left):
                ...
                #sample_id_l = re.search('(\w*)(\_|\_R)([1])(\_|\.f)', cls.path_leaf(left[i])).group(1)
                #sample_id_r = re.search('(\w*)(\_|\_R)([2])(\_|\.f)', cls.path_leaf(right[i])).group(1)
                # urla: replaced original (up) with the following <- worked in minimal test set, should be fine for everything
                sample_id_l = re.search('(.*)_R?1(_|[.]f)', cls.path_leaf(left[i])).group(1)
                sample_id_r = re.search('(.*)_R?2(_|[.]f)', cls.path_leaf(right[i])).group(1)
                print(sample_id_l)
                print(sample_id_r)
