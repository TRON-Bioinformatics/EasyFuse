...
class GTF2DB(object):
    ...
    def create_gffdb(self, overwrite):
        ...
        if os.path.exists(self.db_out_file):
            if overwrite:
                logger.debug(
                    "File {} already exists, but overwrite is specified. Trying to delete the existing file".format(
                        self.db_out_file
                    )
                )
                ...
                logger.debug(
                    "File {} already exists and overwrite is NOT specified.".format(
                        self.db_out_file
                    )
                )
        else:
    ...
    def test_db(self):
        ...
        for feature in gff3_db.featuretypes():
            logger.info(
                "Found {} features of type {} in the db.".format(
                    gff3_db.count_features_of_type(feature), feature
                )
            )
