...
class GTF2DB(object):
    ...
    def create_gffdb(self, overwrite):
        ...
        if os.path.exists(self.db_out_file):
            if overwrite:
                logger.debug("File {} already exists, but overwrite is specified. Trying to delete the existing file".format(
                    self.db_out_file))
                os.remove(self.db_out_file)
                self.create_gffdb(overwrite)
            else:
                logger.debug("File {} already exists and overwrite is NOT specified.".format(self.db_out_file))
        else:
            logger.debug(
                "File {} does not exist. Trying to create it from the provided gff file (This may take 10-20+ minutes)...".format(
                    self.db_out_file))
            # this is a one-time operation and tailored to human ensembl gff3
            gffutils.create_db(self.gff_in_file, self.db_out_file, force_gff=True, force=True,
    ...
    def test_db(self):
        ...
        for feature in gff3_db.featuretypes():
            logger.info("Found {} features of type {} in the db.".format(gff3_db.count_features_of_type(feature), feature))
...
def main():
    ...
    parser.add_argument('--db_output', dest='db_out', help='gff3 derived annotation database from gffutils',
                        required=True)
    parser.add_argument('--overwrite', dest='overwrite', help='Overwrite existing database if present', default=False,
                        action='store_true')
