...
#                    read2 = read
                if not read1 and read.is_read1:
                    ...
                    self.logger.error("Neither r1 nor r2??? Read: {0}; R1: {1}; R2: {2}; bamLine: {3}".format(read, read1, read2, self.counter[0]))
                    sys.exit(1)
    ...
    def print_stats(self):
        ...
        # 7: count_10bp_s_clip
        # 8: count_proper_pair
        self.logger.info("Star_chimeric (chim alignment from star):\t{} pairs (filtered)".format(self.counter[4]))
        self.logger.info("QC'd (additional Star_chimeric alignment):\t{} alignments (included in above)".format(self.counter[5]))
        self.logger.info("Multimapped (1 < x <= 100 equal mappings):\t{} pairs (discarded)".format(self.counter[3]))
        self.logger.info("Unmapped (no mapping or >100 multi map):\t{} pairs (filtered)".format(self.counter[6]))
        self.logger.info("No proper pair (unexpected read distance):\t{} pairs (filtered)".format(self.counter[8]))
        self.logger.info("10bp_s_clip (>9bp soft-clipped in cigar):\t{} pairs (filtered)".format(self.counter[7]))
        ...
        self.logger.info("Not filtered, but mapped to fusion gene:\t{} pairs (filtered)".format(self.counter[9]))
