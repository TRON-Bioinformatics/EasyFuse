...
class TestConfig(TestCase):
    ...
    def test_config_loading(self):
        config_file = pkg_resources.resource_filename(easy_fuse.tests.__name__, "resources/test.config.ini")
        ...
        self.assertEqual(config.get('references', 'genome_fasta'),
                         '/path/to/Homo_sapiens.GRCh38.dna.primary_assembly.fa')
        self.assertEqual(config.get('indices', 'star'), '/path/to/STAR_idx/')
        self.assertEqual(config.get('other_files', 'infusion_cfg'), '/path/to/infusion_index/infusion.cfg')
    ...
    def test_non_existing_config_file(self):
        config_file = pkg_resources.resource_filename(easy_fuse.tests.__name__, "resources/idontexist.config.ini")
