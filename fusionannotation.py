...
class FusionAnnotation(object):
    ...
    def run(self, context_seqs_file, cis_near_distance, genome_fasta, context_seq_len, neo_peptide_len, tsl_filter_level):
        ...
        for record in SeqIO.parse(genome_fasta, "fasta"):
            if record.id in self.cds_seq_dict:
                ...
                self.cds_seq_dict[record.id][1] = [record.seq[max(0, val[0]-1):val[1]] for val in self.cds_seq_dict[record.id][0]]
        ...
        for result_list in results_lists:
            ...
            bp_in_wt2_nt_ex = len(result_list[header_dict["wt2_exon_transcripts"]]) - len(result_list[header_dict["ft2_exon_transcripts"]])
            ...
            result_list[header_dict["neo_peptide_sequence"]] = result_list[header_dict["fusion_peptide"]][max(0, int(bp_in_fusion_aa) - neo_peptide_len):]

            if (int(bp_in_fusion_aa) - neo_peptide_len) < 0:
                result_list[header_dict["neo_peptide_sequence_bp"]] = round(bp_in_fusion_aa, 1)
            else:
                result_list[header_dict["neo_peptide_sequence_bp"]] = round(bp_in_fusion_aa - (int(bp_in_fusion_aa) - neo_peptide_len), 1)
            if result_list[header_dict["frame"]] == "in_frame":
                result_list[header_dict["neo_peptide_sequence"]] = result_list[header_dict["neo_peptide_sequence"]][:(int(result_list[header_dict["neo_peptide_sequence_bp"]]) + neo_peptide_len)]
...
def main():
    ...
    parser.add_argument('--neo_peptide_len', dest='neo_peptide_len', help='The maximal length of neo_peptide at either site of the bp', required=True)
    ...
    fusannot.run(args.out_csv, int(args.cis_near_dist), args.genome_fasta, int(args.context_seq_len), int(args.neo_peptide_len), args.tsl_filter_level)
